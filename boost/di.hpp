...
template <class T, T... Ns> struct integer_sequence {
...
#else
template <class...> struct build_index_sequence;
  ...
  struct scope<TExpected, TGiven,
    ...
    template <class T, class, class TProvider,
              __BOOST_DI_REQUIRES(!detail::is_expr<TGiven, TProvider>::value &&
                                  ...
                                  !aux::is_callable<TExpected>::value) = 0>
    static auto
    try_create(const TProvider &) noexcept -> detail::wrapper_traits_t<
        decltype(aux::declval<typename aux::identity<TGiven, T>::type>()())>;
    template <
